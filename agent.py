...
class SearchAgent:
        ...
        if not self.plan:
            ...
            if self.active_algo == 'BFS':
                ...
                compass_path = self.astar_search(
                    ...
                    tile_facts=self._tile_facts_from_percept(percept)
                )
    ...
    def astar_search(self, start_pos, goal_pos, walls, grid_size,
        ...
        while frontier:
            ...
            for direction, (dx, dy) in self.MOVE_DIRS.items():
                ...
                self.kb.clear_facts()
                for fact in (tile_facts or {}).get(neighbor, ()):
    ...
    @staticmethod
    def _tile_facts_from_percept(percept):
        """Build facts for known tiles while preserving partial observability."""
        tile_facts = {
            tuple(position): list(facts)
            for position, facts in percept.get('tile_facts', {}).items()
        }
        food_positions = set(map(tuple, percept.get('all_food', ())))
        for position in food_positions:
            tile_facts.setdefault(position, []).extend(['HasDust', 'TargetVisible'])
        return tile_facts
